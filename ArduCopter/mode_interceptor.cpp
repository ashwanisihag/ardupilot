...
const AP_Param::GroupInfo ModeIntercept::var_info[] = {
    ...
    // @Units: deg
    // @Range: 5 35
    AP_GROUPINFO("ANG_MAX", 1, ModeIntercept, max_angle_deg, 30.0f),
    ...
    // @Units: -
    AP_GROUPINFO("K_LAT", 3, ModeIntercept, k_lat, 1.1f),
    ...
    // @Param: RLL_RATE
    // @DisplayName: Intercept max roll slew rate
    // @Description: Maximum roll command change rate in degrees per second. Increase to let the vehicle bank into turns faster.
    // @Units: deg/s
    // @Range: 10 180
    AP_GROUPINFO("RLL_RATE", 14, ModeIntercept, roll_rate_max_dps, 80.0f),

    // @Param: PIT_RATE
    // @DisplayName: Intercept max pitch slew rate
    // @Description: Maximum pitch command change rate in degrees per second. Increase to let the vehicle accelerate forward faster.
    // @Units: deg/s
    // @Range: 10 180
    AP_GROUPINFO("PIT_RATE", 15, ModeIntercept, pitch_rate_max_dps, 45.0f),

    // @Param: AUTO_EN
    // @DisplayName: Intercept auto arm/takeoff enable
    // @Description: When set to 1, INTERCEPT will arm and initiate takeoff automatically after a stable target lock.
    // @Values: 0:Disabled,1:Enabled
    AP_GROUPINFO("AUTO_EN", 16, ModeIntercept, auto_takeoff_enable, 0),

    // @Param: LOCK_MS
    // @DisplayName: Intercept lock time before auto action
    // @Description: Continuous target lock duration required before auto arm/takeoff is attempted.
    // @Units: ms
    // @Range: 200 10000
    AP_GROUPINFO("LOCK_MS", 17, ModeIntercept, auto_lock_ms, 1500),

    // @Param: TO_ALT
    // @DisplayName: Intercept auto takeoff altitude
    // @Description: Altitude above current position in meters used when automatic takeoff is initiated from INTERCEPT.
    // @Units: m
    // @Range: 0.5 20
    AP_GROUPINFO("TO_ALT", 18, ModeIntercept, auto_takeoff_alt_m, 2.0f),
...
bool ModeIntercept::init(bool ignore_checks)
{
    ...
    auto_lock_start_ms = 0;
    auto_last_attempt_ms = 0;
    auto_takeoff_issued = false;
    return true;
}
...
void ModeIntercept::run()
{
    ...
    // Optional auto arm + takeoff after sustained lock.
    if (auto_takeoff_enable.get() != 0) {
        if (has_target) {
            if (auto_lock_start_ms == 0) {
                auto_lock_start_ms = now;
            }

            const uint32_t lock_needed_ms = (uint32_t)constrain_int32((int32_t)auto_lock_ms.get(), 200, 10000);
            if (!auto_takeoff_issued && (now - auto_lock_start_ms >= lock_needed_ms)) {
                if ((now - auto_last_attempt_ms) >= 1000U) {
                    auto_last_attempt_ms = now;

                    if (!motors->armed()) {
                        if (!copter.ap.throttle_zero) {
                            GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "INTC: ARM blocked (throttle high)");
                        } else {
                            GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "INTC: ARM start");
                            const bool arm_ok = copter.arming.arm(AP_Arming::Method::MAVLINK);
                            if (!arm_ok) {
                                GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "INTC: ARM failed");
                            }
                        }
                    } else if (copter.ap.land_complete) {
                        const float to_alt_m = constrain_float(auto_takeoff_alt_m.get(), 0.5f, 20.0f);
                        const bool to_ok = do_user_takeoff_U_m(to_alt_m, false);
                        if (to_ok) {
                            auto_takeoff_issued = true;
                        }
                    } else {
                        // Already airborne
                        auto_takeoff_issued = true;
    ...
    const float max_roll_rate_dps  = constrain_float(roll_rate_max_dps.get(), 10.0f, 180.0f);
    const float max_pitch_rate_dps = constrain_float(pitch_rate_max_dps.get(), 10.0f, 180.0f);
