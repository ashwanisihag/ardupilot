...
{
    const float yaw = AP::ahrs().get_yaw_rad(); // radians
...
bool ModeSnake::init(bool ignore_checks)
{
    ...
    _start_yaw_rad = AP::ahrs().get_yaw_rad();
...
void ModeSnake::run()
{
    ...
    case SnakePhase::CALIBRATE: {
        ...
        if (window_done) {
            ...
            if (_remaining_dist_m <= 0.0f) {
                ...
                _phase_start_ms = now;
                _return_yaw_rad = wrap_PI(AP::ahrs().get_yaw_rad()
    ...
    case SnakePhase::TURNAROUND: {
        ...
        const float yaw_now = AP::ahrs().get_yaw_rad();
