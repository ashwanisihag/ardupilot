...
bool ModeAutoCustom::init(bool ignore_checks)
{
    auto_RTL = false;
...
void ModeAutoCustom::set_submode(SubMode new_submode)
{
...
bool ModeAutoCustom::option_is_enabled(Option option) const
{
    return ((copter.g2.auto_options & (uint32_t)option) != 0);
}

bool ModeAutoCustom::allows_arming(AP_Arming::Method method) const
{
    if (auto_RTL) {
        return false;
    }
...
bool ModeAutoCustom::allows_weathervaning() const
{
...
bool ModeAutoCustom::move_vehicle_on_ekf_reset() const
{
        // call the correct auto controller
    switch (_mode) {
    case SubMode::TAKEOFF:
...
bool ModeAutoCustom::jump_to_landing_sequence_auto_RTL(ModeReason reason)
{
...
bool ModeAutoCustom::return_path_start_auto_RTL(ModeReason reason)
{
...
bool ModeAutoCustom::return_path_or_jump_to_landing_sequence_auto_RTL(ModeReason reason)
{
...
bool ModeAutoCustom::enter_auto_rtl(ModeReason reason) 
{
...
bool ModeAutoCustom::nav_script_time(uint16_t &id, uint8_t &cmd, float &arg1, float &arg2, int16_t &arg3, int16_t &arg4)
{
...
void ModeAutoCustom::nav_script_time_done(uint16_t id)
{
#if AP_SCRIPTING_ENABLED
...
bool ModeAutoCustom::loiter_start()
{
    ...
    wp_nav->get_wp_stopping_point_NEU_m(stopping_point_ned_m);

    // initialise waypoint controller target to stopping point
    wp_nav->set_wp_destination_NEU_m(stopping_point_ned_m);
...
void ModeAutoCustom::takeoff_start(const Location& dest_loc)
{
    ...
    if ((dest_loc.get_alt_frame() == Location::AltFrame::ABOVE_TERRAIN) && wp_nav->get_terrain_offset_m(terrain_u_m)) {
...
bool ModeAutoCustom::wp_start(const Location& dest_loc)
{
    ...
    if (!wp_nav->is_active()) {
        ...
        if (_mode == SubMode::TAKEOFF) {
            ...
            if (auto_takeoff.get_completion_pos_neu_m(takeoff_complete_pos_ned_m)) {
...
void ModeAutoCustom::land_start()
{
...
void ModeAutoCustom::circle_movetoedge_start(const Location &circle_center, float radius_m, bool ccw_turn)
{
    // set circle center
    ...
    copter.circle_nav->get_closest_point_on_circle_NEU_m(circle_edge_ned_m, dist_to_edge_m);

    // if more than 3m then fly to edge
    if (dist_to_edge_m > 3.0) {
        // convert circle_edge_ned_m to Location
        Location circle_edge = Location(circle_edge_ned_m, Location::AltFrame::ABOVE_ORIGIN);
...
void ModeAutoCustom::circle_start()
{
...
void ModeAutoCustom::nav_guided_start()
{
...
bool ModeAutoCustom::is_landing() const
{
    switch(_mode) {
    case SubMode::LAND:
        return true;
...
bool ModeAutoCustom::is_taking_off() const
{
    return ((_mode == SubMode::TAKEOFF) && !auto_takeoff.complete);
}

// returns true if pilot's yaw input should be used to adjust vehicle's heading
bool ModeAutoCustom::use_pilot_yaw(void) const
{
...
bool ModeAutoCustom::set_speed_NE_ms(float speed_ne_ms)
{
...
bool ModeAutoCustom::set_speed_up_ms(float speed_up_ms)
{
...
bool ModeAutoCustom::set_speed_down_ms(float speed_down_ms)
{
...
bool ModeAutoCustom::start_command(const AP_Mission::Mission_Command& cmd)
{
    switch(cmd.id) {
...
void ModeAutoCustom::exit_mission()
{
    // play a tone
...
bool ModeAutoCustom::do_guided(const AP_Mission::Mission_Command& cmd)
{
...
float ModeAutoCustom::wp_distance_m() const
{
    switch (_mode) {
    case SubMode::CIRCLE:
...
float ModeAutoCustom::wp_bearing_deg() const
{
    switch (_mode) {
    case SubMode::CIRCLE:
...
bool ModeAutoCustom::get_wp(Location& destination) const
{
    switch (_mode) {
    case SubMode::NAVGUIDED:
...
bool ModeAutoCustom::verify_command(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::takeoff_run()
{
...
//      called by auto_run at 100hz or more
void ModeAutoCustom::circle_run()
{
...
//      called by auto_run at 100hz or more
void ModeAutoCustom::nav_guided_run()
{
...
//      called by auto_run at 100hz or more
void ModeAutoCustom::loiter_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        make_safe_ground_handling();
...
//      called by auto_run at 100hz or more
void ModeAutoCustom::loiter_to_alt_run()
{
...
void ModeAutoCustom::nav_attitude_time_run()
{
    // if not auto armed or motor interlock not enabled set throttle to zero and exit immediately
    if (is_disarmed_or_landed() || !motors->get_interlock()) {
        make_safe_ground_handling();
...
bool ModeAutoCustom::shift_alt_to_current_alt(Location& target_loc) const
{
...
void ModeAutoCustom::subtract_pos_offsets(Location& target_loc) const
{
    // subtract position controller offsets from target location
    const Vector3p& pos_ofs_neu_m = pos_control->get_pos_offset_NEU_m();
    const Vector3p pos_ofs_ned_m(pos_ofs_neu_m.x, pos_ofs_neu_m.y, -pos_ofs_neu_m.z);
...
void ModeAutoCustom::do_takeoff(const AP_Mission::Mission_Command& cmd)
{
...
Location ModeAutoCustom::loc_from_cmd(const AP_Mission::Mission_Command& cmd, const Location& default_loc) const
{
...
void ModeAutoCustom::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
...
bool ModeAutoCustom::set_next_wp(const AP_Mission::Mission_Command& current_cmd, const Location &default_loc)
{
...
void ModeAutoCustom::do_land(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_loiter_unlimited(const AP_Mission::Mission_Command& cmd)
{
    // calculate default location used when lat, lon or alt is zero
    Location default_loc = copter.current_loc;
...
void ModeAutoCustom::do_circle(const AP_Mission::Mission_Command& cmd)
{
    // calculate default location used when lat, lon or alt is zero
    Location default_loc = copter.current_loc;
...
// note: caller should set yaw_mode
void ModeAutoCustom::do_loiter_time(const AP_Mission::Mission_Command& cmd)
{
...
// note: caller should set yaw_mode
void ModeAutoCustom::do_loiter_to_alt(const AP_Mission::Mission_Command& cmd)
{
    // re-use loiter unlimited
    do_loiter_unlimited(cmd);
...
void ModeAutoCustom::do_spline_wp(const AP_Mission::Mission_Command& cmd)
{
    // calculate default location used when lat, lon or alt is zero
    Location default_loc = copter.current_loc;
...
void ModeAutoCustom::get_spline_from_cmd(const AP_Mission::Mission_Command& cmd, const Location& default_loc, Location& dest_loc, Location& next_dest_loc, bool& next_dest_loc_is_spline)
{
...
void ModeAutoCustom::do_nav_guided_enable(const AP_Mission::Mission_Command& cmd)
{
    if (cmd.p1 > 0) {
...
void ModeAutoCustom::do_guided_limits(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_nav_delay(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_nav_script_time(const AP_Mission::Mission_Command& cmd)
{
    // call regular guided flight mode initialisation
    if (copter.mode_guided.init(true)) {
...
void ModeAutoCustom::do_nav_attitude_time(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_wait_delay(const AP_Mission::Mission_Command& cmd)
{
    condition_start = millis();
    condition_value = cmd.content.delay.seconds * 1000;     // convert seconds to milliseconds
}

void ModeAutoCustom::do_within_distance(const AP_Mission::Mission_Command& cmd)
{
    condition_value  = cmd.content.distance.meters;
}

void ModeAutoCustom::do_yaw(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_change_speed(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_set_home(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_roi(const AP_Mission::Mission_Command& cmd)
{
...
#if HAL_MOUNT_ENABLED
// point the camera to a specified angle
void ModeAutoCustom::do_mount_control(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_winch(const AP_Mission::Mission_Command& cmd)
{
...
void ModeAutoCustom::do_payload_place(const AP_Mission::Mission_Command& cmd)
{
    // if location provided we fly to that location at current altitude
    if (cmd.content.location.lat != 0 || cmd.content.location.lng != 0) {
        // set state to fly to location
...
void ModeAutoCustom::do_RTL(void)
{
...
bool ModeAutoCustom::verify_takeoff()
{
#if AP_LANDINGGEAR_ENABLED
...
bool ModeAutoCustom::verify_land()
{
    bool retval = false;
...
bool ModeAutoCustom::verify_loiter_unlimited()
{
    return false;
}

// verify_loiter_time - check if we have loitered long enough
bool ModeAutoCustom::verify_loiter_time(const AP_Mission::Mission_Command& cmd)
{
...
bool ModeAutoCustom::verify_loiter_to_alt() const
{
    if (loiter_to_alt.reached_destination_xy &&
...
bool ModeAutoCustom::verify_RTL()
{
...
bool ModeAutoCustom::verify_wait_delay()
{
...
bool ModeAutoCustom::verify_within_distance()
{
...
bool ModeAutoCustom::verify_yaw()
{
...
bool ModeAutoCustom::verify_nav_wp(const AP_Mission::Mission_Command& cmd)
{
...
bool ModeAutoCustom::verify_circle(const AP_Mission::Mission_Command& cmd)
{
...
bool ModeAutoCustom::verify_spline_wp(const AP_Mission::Mission_Command& cmd)
{
    // check if we have reached the waypoint
    if ( !copter.wp_nav->reached_wp_destination() ) {
        return false;
...
bool ModeAutoCustom::verify_nav_guided_enable(const AP_Mission::Mission_Command& cmd)
{
...
bool ModeAutoCustom::verify_nav_delay(const AP_Mission::Mission_Command& cmd)
{
...
bool ModeAutoCustom::verify_nav_script_time()
{
...
bool ModeAutoCustom::verify_nav_attitude_time(const AP_Mission::Mission_Command& cmd)
{
...
bool ModeAutoCustom::resume()
{
    wp_nav->set_resume();
    return true;
}

bool ModeAutoCustom::paused() const
{
 ...
 */
float ModeAutoCustom::get_alt_above_ground_m() const
{
